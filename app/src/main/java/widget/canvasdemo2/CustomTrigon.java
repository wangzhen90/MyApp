...
public class CustomTrigon extends View {
	...
	public void getDot2(Paint paint, Canvas canvas) {
		//下面公式通过圆心点坐标和柱状体顶部中心点坐标，�?�过二元�?次方程组计算出其余两个三角形的坐标点位置
		...
		//�?始画三角�?
		Path path = new Path();// 三角�?
		
		
		path.moveTo((float)(x2), (float)(y2));//P点坐�?
		path.lineTo((float)(mPointB.x), (float)(mPointB.y));//圆心点坐�?
		path.lineTo((float)x1, (float)y1);//P'点坐�?
		path.close();//闭合画笔路径
		canvas.drawPath(path, paint);//�?始画

	}
	
	/**
	 * 通过不同等级，塞入一个柱状体顶部中心点坐�?
	 * @param pointB
	 */
