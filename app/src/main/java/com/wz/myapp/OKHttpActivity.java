...
//                .url(ApiConfig.HOSTS + "/count/undo-counts.action")
                .url( "https://crm.xiaoshouyi.com/mobile/task/todo-list.action?source=1&os=18&model=generic+vbox86p&cache_key=101&appType=0&_vs=4.2.9")
                .build()

                .enqueue(new JsonCallBack<JsonWork>() {
                    @Override
                    public void onResponse(Call call, JsonWork response) {
                        Log.e("ApiClient", "onResponse,body response:" + response.getScode());
                    }

                    @Override
                    public void onFailure(Call call, Exception error) {
                        Log.e("ApiClient", "onFailure,error:"+error.getMessage());
                    }
                });

//        https://crm.xiaoshouyi.com/mobile/task/todo-list.action?source=1&os=18&model=generic+vbox86p&cache_key=101&appType=0&_vs=4.2.9
    }
...
//        Log.e("Acache", "respons:" + ACache.get().getAsObject("test_json"));
        ...
        try {
            ...
            Gson gson1 = new Gson();
            JsonBase base = gson1.fromJson(jsonStr2,JsonBase.class);
            Log.e("test_json",""+base.scode);
...
//        .cacheType(ApiClient.CACHE_TYPE_CACHE)
                ...
                .enqueue(new JsonCallBack<CacheResult>() {
                    @Override
                    public void onResponse(Call call, CacheResult response) {
    ...
    public void test_post_form(View view) {
        ApiClient.getInstance().postForm()
                ...
                .build()
                .enqueue(new JsonCallBack<LoginModle>() {
                    @Override
                    public void onResponse(Call call, LoginModle response) {
                        Log.e("test_post_form", "response:" + response.head.token);
                    }
    ...
    public void test_post_multi_form(View view){

        File file = new File("/storage/emulated/0/ninestarreader/download_ninestar/1139/unzip/OEBPS/Images/11261991.jpg");
        Log.e("file","file.exists:"+file.exists());
        ApiClient.getInstance().postForm()
                .url("https://crm.xiaoshouyi.com/mobile/feed/multiAdd.action")//如果没有图片，使用的接口是add.action
                .addParams("feedStamp","1467784815656")
                .addParams("content","上传个图片啊，还不行啊")
                .addFile("imgFile","1467786554898.jpg",file)
                .build()
                .enqueue(new JsonCallBack<JsonBase>() {
                    @Override
                    public void onResponse(Call call, JsonBase response) {
                        Log.e("test_post_form", "test_post_multi_form succ,response:" + response.scode);
                    }

                    @Override
                    public void onFailure(Call call, Exception error) {
                        Log.e("test_post_form", "test_post_multi_form failed,,error:" + error.getMessage());
                    }
                });
    }
